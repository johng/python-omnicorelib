...
class Proxy(BaseProxy):
    ...
    def gettxout(self, outpoint, includemempool=True):
        ...
        if r is None:
            raise IndexError(
                "%s.gettxout(): unspent txout %r not found"
                % (self.__class__.__name__, outpoint)
            )

        r["txout"] = CTxOut(
            int(r["value"] * COIN), CScript(unhexlify(r["scriptPubKey"]["hex"]))
        )
        del r["value"]
        del r["scriptPubKey"]
    ...
    def lockunspent(self, unlock, outpoints):
        ...
        json_outpoints = [
            {"txid": b2lx(outpoint.hash), "vout": outpoint.n} for outpoint in outpoints
        ]
    ...
    def sendmany(
        ...
        """
        json_payments = {
            str(addr): float(amount) / COIN for addr, amount in payments.items()
        }
        r = self._call(
            "sendmany",
            fromaccount,
    ...
    def sendtoaddress(
        ...
        r = self._call(
            "sendtoaddress", addr, amount, comment, commentto, subtractfeefromamount
        )
        return lx(r)
